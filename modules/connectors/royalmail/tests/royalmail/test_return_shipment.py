...
class TestRoyalMailClickandDropReturnShipment(unittest.TestCase):
    ...
    @patch("karrio.providers.royalmail.shipment.return_shipment.pycountry", None)
    ...
    def test_create_return_shipment_request_rejects_disallowed_service_from_config(self):
        ...
        shipment = models.ShipmentRequest(**copy.deepcopy(fixture.ReturnShipmentPayload))

        with self.assertRaisesRegex(
            ValueError,
            r"return service is not allowed by `config\.shipping_services`",
        ):
            gateway.mapper.create_return_shipment_request(shipment)

    def test_create_return_shipment_request_rejects_disallowed_shipping_option_from_config(self):
        gateway = karrio.gateway["royalmail"].create(
            {
                "id": "123456789",
                "carrier_id": "royalmail",
        ...
        payload = copy.deepcopy(fixture.ReturnShipmentPayload)
        payload["options"] = {
            "shipment_note": "Not allowed by config",
        }
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"disallowed shipping option\(s\): shipment_note",
        ):
            gateway.mapper.create_return_shipment_request(shipment)