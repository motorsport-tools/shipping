...
import karrio.schemas.royalmail.order_status_request as status_req
import karrio.schemas.royalmail.order_status_response as status_res
...
def _normalize_item(item: typing.Any) -> dict:
    """Normalize an order-status update item to a plain dict."""
    result = {
...
def _validate_item(item: dict) -> None:
    """Validate the required fields for one order-status update item."""
...
def parse_order_status_response(
    ...
    """Parse a Click & Drop order status update response."""
