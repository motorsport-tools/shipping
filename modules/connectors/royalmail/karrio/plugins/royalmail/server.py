...
from karrio.providers.royalmail.utils import (
...
def _get_credentials(instance) -> dict:
    """Return the saved Royal Mail credentials for a carrier connection."""
...
def _has_tracking_credentials(instance) -> bool:
    """Return whether the carrier connection has usable Tracking API credentials."""
...
def _merge_tracking_capability(
    ...
    """Merge the derived tracking capability into a connection config payload."""
    items = [
        capability
...
def get_default_connection_config() -> dict:
    """Build the default Royal Mail connection config exposed by the server extension."""
...
def _populate_default_config(sender, connection_id):
    """Apply default Royal Mail config values before a connection is persisted."""
...
def _sync_tracking_capability(sender, connection_id):
    """Keep the tracking-enabled flag aligned with the saved Tracking API credentials."""
    instance = sender.objects.filter(pk=connection_id).first()
    if instance is None or getattr(instance, "carrier_code", None) != "royalmail":
        return
...
def register():
    """Register Royal Mail server hooks when the Karrio server app is available."""
    global _REGISTERED
    ...
    for model, suffix in [
        ...
        post_save.connect(
            ...
            sender=model,
            dispatch_uid=f"royalmail_populate_default_config_{suffix}",
            weak=False,
        )
        post_save.connect(
            sync_tracking_capability,
            sender=model,
            dispatch_uid=f"royalmail_sync_tracking_capability_{suffix}",
            weak=False,
        )
